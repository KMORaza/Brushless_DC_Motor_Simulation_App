...
namespace BrushlessDCMotorSimulation
{
    ...
    public class BLDCMotor
    {
        ...
        private int phaseLossIndex = -1; // -1 means no phase loss
        private (int, int)? shortCircuitPhases = null; // (phase1, phase2) or null
        ...
        public void ApplyVoltages(double[] voltages, double deltaTime)
        {
            ...
            if (shortCircuitPhases.HasValue)
            {
                ...
                // Short circuit current: V1 = V2, so currents are driven by voltage difference
            ...
            for (int i = 0; i < 3; i++)
            {
                if (i == phaseLossIndex || shortCircuitPhases.HasValue && (i == shortCircuitPhases.Value.Item1 || i == shortCircuitPhases.Value.Item2))
                    continue; // Skip phase loss or short-circuited phases

                double di_dt = (voltages[i] - effectiveResistances[i] * phaseCurrents[i] - backEmf[i]) / mutualInductance;
            ...
            if (phaseLossIndex >= 0)
                phaseCurrents[phaseLossIndex] = 0; // No current in lost phase
        }
        ...
        {
            return torqueConstant * (phaseCurrents[0] + phaseCurrents[1] + phaseCurrents[2]);
        }
        ...
        public void Update(double deltaTime, double loadTorque)
        {
            ...
            torque = Te - loadTorque - GetFrictionTorque();
            double omega = angularVelocityRadPerSec;
            double theta = RotorAngle * Math.PI / 180.0;
            Func<double, double, double, double> dOmega_dt = (t, w, th) => (Te - loadTorque - frictionCoefficient * w) / momentOfInertia;
